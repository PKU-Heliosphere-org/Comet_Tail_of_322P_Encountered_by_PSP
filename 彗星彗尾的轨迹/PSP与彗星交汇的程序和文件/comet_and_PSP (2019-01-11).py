...
def plot_comet_tail(comet_sn_pku_local, number_of_days_comet, number_of_days_psp, beta_lst, date_tuple=(0,0,0), use_jdday=False, jdday=0):
    ...
    beta_min = min(beta_lst)
    beta_max = max(beta_lst)
    h_of_hsv_min = 0.0
    h_of_hsv_max = 270./360.
    h_of_hsv_lst = h_of_hsv_min + (beta_lst-beta_min)/(beta_max-beta_min)*(h_of_hsv_max-h_of_hsv_min)
    s_of_hsv_min = 0.2
    s_of_hsv_max = 1.0
    day_vect_comet = list(range(number_of_days_comet))
    s_of_hsv_lst = s_of_hsv_max - (day_vect_comet)/number_of_days_comet*(s_of_hsv_max-s_of_hsv_min)
    v_of_hsv_lst = beta_lst*0.0+1.0
