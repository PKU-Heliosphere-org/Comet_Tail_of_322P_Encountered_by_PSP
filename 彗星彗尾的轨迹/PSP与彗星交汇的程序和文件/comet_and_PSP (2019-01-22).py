...
'''
file_dir = '/Users/duo/Desktop/My Files/太阳风小小组/太阳系小天体/'
f = open(file_dir+'cometels_e_less_than_1.json')
ele_data = json.load(f)
f.close()

f2 = open(file_dir+'modified_PSP_ephemeris_data.json')
...
def single_beta_tail_particles(comet_sn_pku_local, number_of_days, number_of_time_blocks, beta, date_tuple=(0,0,0), use_jdday=False, jdday=0):
    """
...
def dist_psp_particle(comet_sn_pku_local, number_of_days_comet, number_of_time_blocks_comet, beta_lst, date_tuple=(0,0,0), use_jdday=False, jdday=0):
    """
    :param comet_sn_pku_local:
    ...
    # psp_particle_dist_arr是一个二维数组，行从上到下为不同的beta，列从左到右为回溯天数由小到大对应的尘埃和psp的距离
    psp_present_pos = psp_time_pos[str(jd_day_for_psp)][1:4] # psp_present_pos是psp当前（date_tuple或jdday）的位置
    i_beta = 0
    for beta in beta_lst:
        ...
        for single_pos in particles_present_pos:
            dist = math.sqrt((psp_present_pos[0] - single_pos[0])**2 + (psp_present_pos[1] - single_pos[1])**2 +
                             (psp_present_pos[2] - single_pos[2])**2)
            psp_particle_dist_arr[i_beta][-i_day - 1] = dist
            i_day = i_day + 1
        i_beta = i_beta + 1
    min_dist_psp_particle = np.min(psp_particle_dist_arr)
    min_dist_iday = np.where(psp_particle_dist_arr == np.min(psp_particle_dist_arr))[1][0]
    min_dist_ibeta = np.where(psp_particle_dist_arr == np.min(psp_particle_dist_arr))[0][0]
    return min_dist_iday, min_dist_ibeta, min_dist_psp_particle, psp_particle_dist_arr
...
if __name__ == '__main__':
    ...
    print(single_beta_tail_particles(6, 80, 640, 0.2, (2019, 9, 2)))

    print(dist_psp_particle(6, 80, 640, [0.2, 0.4, 0.6], date_tuple=(2019, 9, 2)))
