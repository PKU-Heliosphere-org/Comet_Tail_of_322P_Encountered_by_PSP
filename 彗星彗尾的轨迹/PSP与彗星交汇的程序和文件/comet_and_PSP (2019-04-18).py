...
def generate_ephemeris_file(comet_ecliptic, comet_carrington, psp_carrington):
    """
    生成星历文件
    :return:
    """
    if psp_carrington:
        file_psp_ephemeris_2 = open("psp_ephemeris_heliocentric_carrington.txt", "w")
        file_psp_ephemeris_2.write("PSP ephemeris in heliographic carrington\n\n")

        the_psp_pos_ecliptic_lst = []
        the_psp_pos_ecliptic_before_short_time_lst = []
        the_psp_pos_ecliptic_after_short_time_lst = []
        the_jdday_lst = []
        the_date_lst = []
        the_time_lst = []
        the_date_before_lst = []
        ...
        the_time_before_lst = []
        the_time_after_lst = []
        for time_sn_i in range(9072, 17856):
            the_jdday = psp_time_pos[time_sn_i]["jd_day"]
            the_date = psp_time_pos[time_sn_i]["date"]
            the_time = psp_time_pos[time_sn_i]["time"]
            the_date_before = psp_time_pos[time_sn_i - 1]["date"]
            the_date_after = psp_time_pos[time_sn_i + 1]["date"]
            the_time_before = psp_time_pos[time_sn_i - 1]["time"]
            the_time_after = psp_time_pos[time_sn_i + 1]["time"]

            the_psp_pos_ecliptic = [psp_time_pos[time_sn_i]["x"], psp_time_pos[time_sn_i]["y"],
                                    psp_time_pos[time_sn_i]["z"]]
            the_psp_pos_ecliptic_before_short_time = [psp_time_pos[time_sn_i - 1]["x"],
                                                      psp_time_pos[time_sn_i - 1]["y"],
                                                      psp_time_pos[time_sn_i - 1]["z"]]
            the_psp_pos_ecliptic_after_short_time = [psp_time_pos[time_sn_i + 1]["x"], psp_time_pos[time_sn_i + 1]["y"],
                                                     psp_time_pos[time_sn_i + 1]["z"]]

            the_psp_pos_ecliptic_lst.append(the_psp_pos_ecliptic)
            the_psp_pos_ecliptic_before_short_time_lst.append(the_psp_pos_ecliptic_before_short_time)
            the_psp_pos_ecliptic_after_short_time_lst.append(the_psp_pos_ecliptic_after_short_time)
            the_jdday_lst.append(the_jdday)
            the_date_lst.append(the_date)
            the_time_lst.append(the_time)
            the_date_before_lst.append(the_date_before)
            the_date_after_lst.append(the_date_after)
            the_time_before_lst.append(the_time_before)
            the_time_after_lst.append(the_time_after)

        the_psp_coord_ecliptic_lst = SkyCoord([single_pos[0] for single_pos in the_psp_pos_ecliptic_lst] * u.AU,
                                              [single_pos[1] for single_pos in the_psp_pos_ecliptic_lst] * u.AU,
                                              [single_pos[2] for single_pos in the_psp_pos_ecliptic_lst] * u.AU,
                                              representation_type="cartesian", frame="heliocentrictrueecliptic",
                                              obstime=[date_english2num(the_date_lst[j]) + " " + the_time_lst[j] for j
                                                       in
                                                       range(len(the_date_lst))])
        the_psp_coord_ecliptic_before_short_time_lst = SkyCoord(
            [single_pos[0] for single_pos in the_psp_pos_ecliptic_before_short_time_lst] * u.AU,
            [single_pos[1] for single_pos in the_psp_pos_ecliptic_before_short_time_lst] * u.AU,
            [single_pos[2] for single_pos in the_psp_pos_ecliptic_before_short_time_lst] * u.AU,
            representation_type="cartesian", frame="heliocentrictrueecliptic",
            obstime=[date_english2num(the_date_before_lst[j]) + " " + the_time_before_lst[j] for j in
                     range(len(the_date_before_lst))])
        the_psp_coord_ecliptic_after_short_time_lst = SkyCoord(
            [single_pos[0] for single_pos in the_psp_pos_ecliptic_after_short_time_lst] * u.AU,
            [single_pos[1] for single_pos in the_psp_pos_ecliptic_after_short_time_lst] * u.AU,
            [single_pos[2] for single_pos in the_psp_pos_ecliptic_after_short_time_lst] * u.AU,
            representation_type="cartesian", frame="heliocentrictrueecliptic",
            obstime=[date_english2num(the_date_after_lst[j]) + " " + the_time_after_lst[j] for j in
                     range(len(the_date_after_lst))])
        the_psp_coord_carrington_lst = the_psp_coord_ecliptic_lst.transform_to(frames.HeliographicCarrington)
        the_psp_coord_carrington_before_short_time_lst = the_psp_coord_ecliptic_before_short_time_lst.transform_to(
            frames.HeliographicCarrington)
        the_psp_coord_carrington_after_short_time_lst = the_psp_coord_ecliptic_after_short_time_lst.transform_to(
            frames.HeliographicCarrington)

        for i in range(len(the_date_lst)):
            the_psp_v_carrington_lon = (
                                               the_psp_coord_carrington_after_short_time_lst[i].lon.value -
                                               the_psp_coord_carrington_before_short_time_lst[i].lon.value) * 72
            the_psp_v_carrington_lat = (
                                               the_psp_coord_carrington_after_short_time_lst[i].lat.value -
                                               the_psp_coord_carrington_before_short_time_lst[i].lat.value) * 72
            the_psp_v_carrington_radius = (
                                                  the_psp_coord_carrington_after_short_time_lst[i].radius.value -
                                                  the_psp_coord_carrington_before_short_time_lst[i].radius.value) * 72
            file_psp_ephemeris_2.write(str(the_jdday_lst[i]) + " " + the_date_lst[i] + " " + the_time_lst[i] + "\n")
            file_psp_ephemeris_2.write(
                " LON =" + str(the_psp_coord_carrington_lst[i].lon.value) + " LAT =" + str(
                    the_psp_coord_carrington_lst[i].lat.value)
                + " RADIUS =" + str(the_psp_coord_carrington_lst[i].radius.value) + "\n")
            file_psp_ephemeris_2.write(
                " V_LON=" + str(the_psp_v_carrington_lon) + " V_LAT=" + str(the_psp_v_carrington_lat)
                + " V_RADIUS=" + str(the_psp_v_carrington_radius) + "\n\n")
        file_psp_ephemeris_2.close()

    elif comet_carrington:
        file_comet_ephemeris_2 = open("comet_ephemeris_heliographic_carrington_V2.txt", "w")
        file_comet_ephemeris_2.write("comet ephemeris in heliographic carrington V2\n\n")
        the_comet_pos_ecliptic_lst = []
        the_comet_pos_ecliptic_before_short_time_lst = []
        the_comet_pos_ecliptic_after_short_time_lst = []
        the_jdday_lst = []
        the_date_lst = []
        the_time_lst = []
        the_date_before_lst = []
        ...
        for time_sn_i in range(9072, 17856):
            ...
            the_time_before = psp_time_pos[time_sn_i - 1]["time"]
            the_time_after = psp_time_pos[time_sn_i + 1]["time"]
            the_jdday_before_short_time = psp_time_pos[time_sn_i - 1]["jd_day"]
            the_jdday_after_short_time = psp_time_pos[time_sn_i + 1]["jd_day"]

            the_comet_pos_ecliptic = get_pos_of_the_obj(6, the_jdday)[0][:3]
            the_comet_pos_ecliptic_before_short_time = get_pos_of_the_obj(6, the_jdday_before_short_time)[0][:3]
            the_comet_pos_ecliptic_after_short_time = get_pos_of_the_obj(6, the_jdday_after_short_time)[0][:3]

            the_comet_pos_ecliptic_lst.append(the_comet_pos_ecliptic)
            the_comet_pos_ecliptic_before_short_time_lst.append(the_comet_pos_ecliptic_before_short_time)
            the_comet_pos_ecliptic_after_short_time_lst.append(the_comet_pos_ecliptic_after_short_time)
            the_jdday_lst.append(the_jdday)
            the_date_lst.append(the_date)
            the_time_lst.append(the_time)
            the_date_before_lst.append(the_date_before)
        ...
        the_comet_coord_ecliptic_lst = SkyCoord([single_pos[0] for single_pos in the_comet_pos_ecliptic_lst] * u.AU,
                                                [single_pos[1] for single_pos in the_comet_pos_ecliptic_lst] * u.AU,
                                                [single_pos[2] for single_pos in the_comet_pos_ecliptic_lst] * u.AU,
                                                representation_type="cartesian", frame="heliocentrictrueecliptic",
                                                obstime=[date_english2num(the_date_lst[j]) + " " + the_time_lst[j] for j
                                                         in range(len(the_date_lst))])
        the_comet_coord_ecliptic_before_short_time_lst = SkyCoord(
            [single_pos[0] for single_pos in the_comet_pos_ecliptic_before_short_time_lst] * u.AU,
            [single_pos[1] for single_pos in the_comet_pos_ecliptic_before_short_time_lst] * u.AU,
            [single_pos[2] for single_pos in the_comet_pos_ecliptic_before_short_time_lst] * u.AU,
            representation_type="cartesian", frame="heliocentrictrueecliptic",
            obstime=[date_english2num(the_date_before_lst[j]) + " " + the_time_before_lst[j] for j in
                     range(len(the_date_before_lst))])
        the_comet_coord_ecliptic_after_short_time_lst = SkyCoord(
            [single_pos[0] for single_pos in the_comet_pos_ecliptic_after_short_time_lst] * u.AU,
            [single_pos[1] for single_pos in the_comet_pos_ecliptic_after_short_time_lst] * u.AU,
            [single_pos[2] for single_pos in the_comet_pos_ecliptic_after_short_time_lst] * u.AU,
            representation_type="cartesian", frame="heliocentrictrueecliptic",
            obstime=[date_english2num(the_date_after_lst[j]) + " " + the_time_after_lst[j] for j in
                     range(len(the_date_after_lst))])
        the_comet_coord_carrington_lst = the_comet_coord_ecliptic_lst.transform_to(frames.HeliographicCarrington)
        the_comet_coord_carrington_before_short_time_lst = the_comet_coord_ecliptic_before_short_time_lst.transform_to(
            frames.HeliographicCarrington)
        the_comet_coord_carrington_after_short_time_lst = the_comet_coord_ecliptic_after_short_time_lst.transform_to(
            frames.HeliographicCarrington)

        for i in range(len(the_date_lst)):
            the_comet_v_carrington_lon = (
                                                 the_comet_coord_carrington_after_short_time_lst[i].lon.value -
                                                 the_comet_coord_carrington_before_short_time_lst[i].lon.value) * 72
            the_comet_v_carrington_lat = (
                                                 the_comet_coord_carrington_after_short_time_lst[i].lat.value -
                                                 the_comet_coord_carrington_before_short_time_lst[i].lat.value) * 72
            the_comet_v_carrington_radius = (
                                                    the_comet_coord_carrington_after_short_time_lst[i].radius.value -
                                                    the_comet_coord_carrington_before_short_time_lst[
                                                        i].radius.value) * 72
            file_comet_ephemeris_2.write(str(the_jdday_lst[i]) + " " + the_date_lst[i] + " " + the_time_lst[i] + "\n")
            file_comet_ephemeris_2.write(
                " LON =" + str(the_comet_coord_carrington_lst[i].lon.value) + " LAT =" + str(
                    the_comet_coord_carrington_lst[i].lat.value)
                + " RADIUS =" + str(the_comet_coord_carrington_lst[i].radius.value) + "\n")
            file_comet_ephemeris_2.write(
                " V_LON=" + str(the_comet_v_carrington_lon) + " V_LAT=" + str(the_comet_v_carrington_lat)
                + " V_RADIUS=" + str(the_comet_v_carrington_radius) + "\n\n")
        file_comet_ephemeris_2.close()

    else:
        file_comet_ephemeris_1 = open("comet_ephemeris_heliocentric_ecliptic.txt", "w")
        file_comet_ephemeris_1.write("comet ephemeris in heliocentric ecliptic\n\n")
        for time_sn_i in range(9072, 17856):
            the_jdday = psp_time_pos[time_sn_i]["jd_day"]
            the_date = psp_time_pos[time_sn_i]["date"]
            the_time = psp_time_pos[time_sn_i]["time"]
            ...
            the_comet_pos_ecliptic_before_short_time = get_pos_of_the_obj(6, the_jdday_before_short_time)[0][:3]
            the_comet_pos_ecliptic_after_short_time = get_pos_of_the_obj(6, the_jdday_after_short_time)[0][:3]
            the_comet_v_ecliptic = tuple(
                (the_comet_pos_ecliptic_after_short_time[i] - the_comet_pos_ecliptic_before_short_time[i]) * 72 for i in
                range(3))
            file_comet_ephemeris_1.write(str(the_jdday) + " " + the_date + " " + the_time + "\n")
            file_comet_ephemeris_1.write(
                " X =" + str(the_comet_pos_ecliptic[0]) + " Y =" + str(the_comet_pos_ecliptic[1]) + " Z =" + str(
                    the_comet_pos_ecliptic[2]) + "\n")
            file_comet_ephemeris_1.write(
                " VX=" + str(the_comet_v_ecliptic[0]) + " VY=" + str(the_comet_v_ecliptic[1]) + " VZ=" + str(
                    the_comet_v_ecliptic[2]) + "\n\n")
        file_comet_ephemeris_1.close()
...
if __name__ == '__main__':
    ...
    # 9072 13442 17856
    # generate_ephemeris_file(psp_carrington=True)
